...
func getIconHandler(c echo.Context) error {
	...
	var user UserModel
	if err := tx.GetContext(ctx, &user, "SELECT * FROM users WHERE name = ? LIMIT 1", username); err != nil {
	...
	if user.IconHash != "" && user.IconHash == c.Request().Header.Get("If-None-Match") {
		return c.NoContent(http.StatusNotModified)
	}

	var image []byte
	if err := tx.GetContext(ctx, &image, "SELECT image FROM icons WHERE user_id = ? LIMIT 1", user.ID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func getMeHandler(c echo.Context) error {
	...
	userModel := UserModel{}
	err = tx.GetContext(ctx, &userModel, "SELECT * FROM users WHERE id = ? LIMIT 1", userID)
	if errors.Is(err, sql.ErrNoRows) {
...
func loginHandler(c echo.Context) error {
	...
	err = tx.GetContext(ctx, &userModel, "SELECT * FROM users WHERE name = ? LIMIT 1", req.Username)
	if errors.Is(err, sql.ErrNoRows) {
...
func getUserHandler(c echo.Context) error {
	...
	userModel := UserModel{}
	if err := tx.GetContext(ctx, &userModel, "SELECT * FROM users WHERE name = ? LIMIT 1", username); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusNotFound, "not found user that has the given username")
		}
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get user: "+err.Error())
...
func fillUserResponse(ctx context.Context, tx *sqlx.Tx, userModel UserModel) (User, error) {
	themeModel := ThemeModel{}
	if err := tx.GetContext(ctx, &themeModel, "SELECT * FROM themes WHERE user_id = ? LIMIT 1", userModel.ID); err != nil {
		return User{}, err
	}
